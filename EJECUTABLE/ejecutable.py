 ...
 while seguir:
  ...
  if opc == '1':
    for clave, valor in flota.items():
     ...
     vehiculo_seleccionado = flota[str(vehiculo)]

     reserva = Reserva(contador_reservas,vehiculo_seleccionado,dni_cliente, fecha_inicio, fecha_fin, tipo_licencia, destino)
     vehiculo_seleccionado.alquilar()
     print(f"Contrato RES-00{contador_reservas} creado. Vehículo {vehiculo_seleccionado.matricula} marcado como alquilado.")
     contador_reservas += 1
